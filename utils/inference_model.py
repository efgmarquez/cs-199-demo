...
os.environ["HF_TOKEN"] = os.getenv("HUGGINGFACE_TOKEN")  # set HF_TOKEN for transformer_lens


# Constants
DTYPES = {"fp32": torch.float32, "fp16": torch.float16, "bf16": torch.bfloat16}

def load_crosscoder(repo_id: str, path: str = "blocks.14.hook_resid_pre", device: str = "cpu"):
...
class CrossCoder(nn.Module):
    def __init__(self, cfg):
        ...
        self.W_dec = nn.Parameter(torch.nn.init.normal_(torch.empty(d_hidden, 2, d_in, dtype=self.dtype)))
        self.W_dec.data = self.W_dec.data / self.W_dec.data.norm(dim=-1, keepdim=True) * self.cfg["dec_init_norm"]
        self.W_enc.data = einops.rearrange(self.W_dec.data.clone(), "d_hidden n_models d_model -> n_models d_model d_hidden")
    ...
    def encode(self, x, apply_relu=True):
        x_enc = einops.einsum(x, self.W_enc, "batch n_models d_model, n_models d_model d_hidden -> batch d_hidden")
        return F.relu(x_enc + self.b_enc) if apply_relu else x_enc + self.b_enc

    def decode(self, acts):
        acts_dec = einops.einsum(acts, self.W_dec, "batch d_hidden, d_hidden n_models d_model -> batch n_models d_model")
...
def tokenize_instructions_gemma(tokenizer: AutoTokenizer, instructions: List[str]) -> Tensor:
    return tokenizer(instructions, padding=True, truncation=False, return_tensors="pt").input_ids
